...
def setting_1__directly_on_listops(model_cls, model_kwargs, batch_size=256):
    ...
    test_dataset = ListOpsDataset(split='test', tokenizer=tokenizer, n_samples=10, task='classification')
...
def setting_2__clm_pretrain_text_then_listops(model_cls, model_kwargs, batch_size=256):
    ...
    # 1.A. Text:
    text_dataset = OpenWebTextDataset(n_samples=2000)
    tokenizer = text_dataset.tokenizer
    vocab_size = tokenizer.vocab_size
    # 1.B. ListOps:
    listops_train_dataset = ListOpsDataset(split='train', tokenizer=tokenizer, n_samples=2000, task='classification')
    ...
    text_test_dataloader = DataLoader(text_dataset, batch_size=batch_size, shuffle=True)  # TODO test-set
...
def setting_3__clm_pretrain_listops_then_listops(model_cls, model_kwargs, batch_size=256):
    # 1. Load data and init model
    listops_train_dataset = ListOpsDataset(split='train', n_samples=2000, task='auto_regressive')
